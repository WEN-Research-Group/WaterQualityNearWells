...
SW_WQP_Ba_CO = filter(SW_WQP_anal_daily, daily_mean < 6000 & daily_mean > 1 & max_v/min_v < 100)
View(SW_WQP_Ba_CO)
...
SW_COGCC_Ba_CO = filter(cogcc_wq_curated_daily, daily_mean < 500 & max_v/min_v < 100)


#Merge two datasets together and save
SW_Ba_CO_merged = rbind(SW_COGCC_Ba_CO, SW_WQP_Ba_CO)
View(SW_Ba_CO_merged)

#Save this for later
saveRDS(SW_Ba_CO_merged,file = "SW_all_Ba_CO_merged.rds", compress = FALSE)
...
#Analyze outliers
View(cogcc_wq_curated_daily)
GW_COGCC_Ba_CO = filter(cogcc_wq_curated_daily, daily_mean < 2000 & max_v/min_v < 100)
...
#Remove a few largest values
GW_WQP_Ba_CO = filter(GW_WQP_anal_daily, daily_mean < 6000 & daily_mean > 1 & max_v/min_v < 100)

#Merge two datasets together and save
GW_Ba_CO_merged = rbind(GW_COGCC_Ba_CO, GW_WQP_Ba_CO)

#Save this for later
saveRDS(GW_Ba_CO_merged,file = "GW_all_Ba_CO_merged.rds", compress = FALSE)
...
surface_water_data = SW_Ba_CO_merged
groundwater_data = GW_Ba_CO_merged
...
Geospatial_result_CO_Ba = rbind(SW_marg,SW_unconv,SW_orph,GW_marg,GW_unconv,GW_orph)

saveRDS(Geospatial_result_CO_Ba,file = "Geospatial_results_CO_Ba.rds", compress = FALSE)
