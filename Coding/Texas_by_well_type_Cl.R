...
Texas_state_Cl = Cleaning_TX_SW_fun_daily(TX_SW_Cl,TX_SW_sites)
Texas_state_Cl_non_USGS = filter(Texas_state_Cl,USGS_GAUGE == "")


#Water quality portal
#All data - need to clean it still
SW_WQP_Cl = readRDS("./input/sw_results_all_info_chloride.rds")

#Look up these to include in the filter
unique(SW_WQP_Cl$ResultDetectionConditionText)
unique(SW_WQP_Cl$MonitoringLocationTypeName)
unique(SW_WQP_Cl$ResultStatusIdentifier)
unique(SW_WQP_Cl$ResultMeasure.MeasureUnitCode)

#Cleaning Na data for all the states
SW_WQP_Cl_clean = SW_WQP_Cl %>%
...
unique(SW_WQP_Cl_clean$ResultDetectionConditionText)
unique(SW_WQP_Cl_clean$MonitoringLocationTypeName)
unique(SW_WQP_Cl_clean$ResultStatusIdentifier)
unique(SW_WQP_Cl_clean$ResultMeasure.MeasureUnitCode)

#Do unit conversions 
ind = which(SW_WQP_Cl_clean$ResultMeasure.MeasureUnitCode == "mg/L"|
              SW_WQP_Cl_clean$ResultMeasure.MeasureUnitCode == "mg/l"|
              SW_WQP_Cl_clean$ResultMeasure.MeasureUnitCode == "ppm")

SW_WQP_Cl_clean[ind,"DataValue"] = SW_WQP_Cl_clean[ind,"DataValue"]*1000
#All values are converted and now ug/L
SW_WQP_Cl_clean$ResultMeasure.MeasureUnitCode = "ug/L"

#Save file to use with other states
saveRDS(SW_WQP_Cl_clean, file="sw_wqp_wq_all_curated_Cl.rds", compress = FALSE)


#Separate Texas samples
SW_WQP_Cl_Texas = filter(SW_WQP_Cl_clean, State=="Texas")

#Calculate daily values
SW_WQP_Cl_Texas_daily = SW_WQP_Cl_Texas %>%
...
Texas_state_Cl_non_USGS$SiteCode = as.character(Texas_state_Cl_non_USGS$SiteCode)
#Remove some columns 
Texas_state_Cl_non_USGS = Texas_state_Cl_non_USGS[,-c(3,6)]

#Now merge
SW_Cl_Texas_merged = rbind(Texas_state_Cl_non_USGS,SW_WQP_Cl_Texas_daily)

#Check for outliers
SW_Cl_Texas_merged_ord = SW_Cl_Texas_merged %>%
  arrange(daily_mean)

#Remove largest values
SW_Cl_Texas_merged = filter(SW_Cl_Texas_merged, daily_mean < 200000000)

Q_75 = quantile(SW_Cl_Texas_merged$daily_mean, probs = c(0.75)) # that would be 32,000

dim(SW_Cl_Texas_merged)
Q_75

#Save this for later
saveRDS(SW_Cl_Texas_merged,file = "SW_daily_Cl_Texas_merged.rds", compress = FALSE)
...
#WQP
GW_WQP_Cl_clean = readRDS("gw_wqp_wq_all_curated_Cl.rds")

GW_WQP_Cl_clean_TX = filter(GW_WQP_Cl_clean, State == "Texas")

GW_WQP_Cl_clean_TX_daily = GW_WQP_Cl_clean_TX %>%
  group_by(County, SiteCode, Latitude, Longitude, Samplingdate) %>%
  summarise(n = n(), daily_mean = mean(DataValue), daily_median = median(DataValue), 
            min_v = min(DataValue),max_v = max(DataValue)) %>%
  arrange(daily_mean)
View(GW_WQP_Cl_clean_TX_daily)
#Some values are very large - remove values below 104333333 
GW_WQP_Cl_clean_TX_daily = filter(GW_WQP_Cl_clean_TX_daily, daily_mean < 100000000)
...
gw.twdb.cl_long_clean <- wqdetail %>%
  filter(WaterQualityParameterId %in% c(80, 81),#look up these values for each analyte
         is.na(Description),
         ParameterValue> 1) %>%
  mutate(SampleDate = 
  ...
  arrange(DataValue)
View(gw.twdb.cl_long_clean)

#Check units
unique(gw.twdb.cl_long_clean$ParameterLongDescription)

#Change units
gw.twdb.cl_long_clean$DataValue = gw.twdb.cl_long_clean$DataValue * 1000

#gw.twdb.cl_long_clean = gw.twdb.cl_long %>%
...
gw_twdb_cl_clean = gw.twdb.cl_long_clean %>%
  select(c(County, SiteCode, Latitude, Longitude, Samplingdate, DataValue))

#Calculate daily averages 
TX_GW_TWDB_Cl_part1 = gw_twdb_cl_clean %>%
  group_by(County, SiteCode, Latitude, Longitude, Samplingdate) %>%
  summarise(n = n(), daily_mean = mean(DataValue), daily_median = median(DataValue), 
            min_v = min(DataValue),max_v = max(DataValue)) %>%
  mutate(SiteCode = as.character(SiteCode)) %>%
  arrange(daily_mean)
View(TX_GW_TWDB_Cl_part1)

#Remove one largest value - clearly an outlier
TX_GW_TWDB_Cl_part1 = TX_GW_TWDB_Cl_part1[-dim(TX_GW_TWDB_Cl_part1)[1],]
...
GW_Cl_Texas_merged = rbind(TX_GW_TWDB_Cl_part1,GW_WQP_Cl_clean_TX_daily)

#Check for outliers
GW_Cl_Texas_merged_org = GW_Cl_Texas_merged %>%
  arrange(daily_mean)

View(GW_Cl_Texas_merged_org)
#Remove outliers if needed

Q_75 = quantile(GW_Cl_Texas_merged_org$daily_mean, probs = c(0.75)) # that would be 32,000

dim(GW_Cl_Texas_merged_org)
Q_75

#Save this for later
saveRDS(GW_Cl_Texas_merged,file = "GW_all_Cl_Texas_merged.rds", compress = FALSE)
...
#MARGINAL
sw.wq.distance = Well_spatial_analysis_compl_0127(Texas_marginal_short,SW_Cl_Texas_merged)
...
sw.wq.distance = Well_spatial_analysis_compl_0127(enverus_og_well_curated,SW_Cl_Texas_merged)
...
sw.wq.distance = Well_spatial_analysis_compl_0127(Texas_orphaned_clean,SW_Cl_Texas_merged)
well_type = "orphaned"
Saving_figures_fun_exp(sw.wq.distance,anal, meas, state, type_w, well_type)
...
#MARGINAL
gw.wq.distance = Well_spatial_analysis_compl_0127(Texas_marginal_short,GW_Cl_Texas_merged)
...
gw.wq.distance = Well_spatial_analysis_compl_0127(enverus_og_well_curated,GW_Cl_Texas_merged)
well_type = "unconventional"
Saving_figures_fun_exp(gw.wq.distance,anal, meas, state, type_w, well_type)
...
gw.wq.distance = Well_spatial_analysis_compl_0127(Texas_orphaned_clean,GW_Cl_Texas_merged)
well_type = "orphaned"
Saving_figures_fun_exp(gw.wq.distance,anal, meas, state, type_w, well_type)
...
Geospatial_result_TX_Cl = rbind(SW_marg,SW_unconv,SW_orph,GW_marg,GW_unconv,GW_orph)

saveRDS(Geospatial_result_TX_Cl,file = "Geospatial_results_TX_Cl.rds", compress = FALSE)
