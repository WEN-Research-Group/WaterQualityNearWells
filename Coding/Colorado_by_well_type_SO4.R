...
SW_WQP_SO4_CO = filter(SW_WQP_anal_daily, daily_mean < 15000000 & daily_mean > 1 & max_v/min_v < 100)
...
SW_COGCC_SO4_CO = filter(cogcc_wq_curated_daily, daily_mean < 10000000 & max_v/min_v < 100)


#Merge two datasets together and save
SW_SO4_CO_merged = rbind(SW_COGCC_SO4_CO, SW_WQP_SO4_CO)
View(SW_SO4_CO_merged)

#Save this for later
saveRDS(SW_SO4_CO_merged,file = "SW_all_SO4_CO_merged.rds", compress = FALSE)


#Mapping
SW_SO4_CO_merged_org = SW_SO4_CO_merged %>%
  arrange(daily_mean)
View(SW_SO4_CO_merged_org) # no weird coordinates 
...
Q_75 = quantile(SW_SO4_CO_merged_org$daily_mean, probs = c(0.75)) # that would be 32,000
data_perc_75 = filter(SW_SO4_CO_merged_org, daily_mean > Q_75 & Longitude < -20 )
...
#Analyze outliers
View(cogcc_wq_curated_daily)
GW_COGCC_SO4_CO = filter(cogcc_wq_curated_daily, daily_mean < 5000000 & max_v/min_v < 100)
...
#Remove a few largest values
GW_WQP_SO4_CO = filter(GW_WQP_anal_daily, daily_mean < 70000000 & max_v/min_v < 100)

#Merge two datasets together and save
GW_SO4_CO_merged = rbind(GW_COGCC_SO4_CO, GW_WQP_SO4_CO)

#Save this for later
saveRDS(GW_SO4_CO_merged,file = "GW_all_SO4_CO_merged.rds", compress = FALSE)


#Mapping
GW_SO4_CO_merged_org = GW_SO4_CO_merged %>%
  arrange(daily_mean)
View(GW_SO4_CO_merged_org) # no weird coordinates 
#Filter out two values with weird coordinates 
#GW_Cl_CO_merged_org = filter(GW_Cl_CO_merged_org, Latitude < 80)
#ind = which(GW_Cl_CO_merged_org$Latitude > 80)
#GW_Cl_CO_merged_org = GW_Cl_CO_merged_org[-ind,]
Q_75 = quantile(GW_SO4_CO_merged_org$daily_mean, probs = c(0.75)) # that would be 32,000
data_perc_75 = filter(GW_SO4_CO_merged_org, daily_mean > Q_75 & Longitude < -20 & Latitude < 80)
#data_perc_75_low = filter(GW_Cl_CO_merged_org, daily_mean < Q_75 & Longitude < -20)
#data_perc_75 = filter(GW_WQP_SO4_NY, daily_mean > quantile(GW_WQP_SO4_NY$daily_mean, probs = c(0.75)))
pal <- colorNumeric(palette = "viridis", domain = data_perc_75$daily_mean)
#map_75 = 
...
surface_water_data = SW_SO4_CO_merged
groundwater_data = GW_SO4_CO_merged
...
Geospatial_result_CO_SO4 = rbind(SW_marg,SW_unconv,SW_orph,GW_marg,GW_unconv,GW_orph)

saveRDS(Geospatial_result_CO_SO4,file = "Geospatial_results_CO_SO4.rds", compress = FALSE)
saveRDS(Geospatial_result_CO_SO4,file = "Geospatial_results_CO_SO4_all.rds", compress = FALSE)
