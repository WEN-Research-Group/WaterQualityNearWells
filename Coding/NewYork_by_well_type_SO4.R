...
SW_WQP_SO4_State = filter(SW_WQP_SO4, State=="New York")

#Calculate daily values
SW_WQP_SO4_NY_daily = SW_WQP_SO4_State %>%
...
View(SW_WQP_SO4_NY_daily)
#No outliers
SW_WQP_SO4_NY = SW_WQP_SO4_NY_daily

#Find 75th percentile? Then plot values above that value?
Q_75 = quantile(SW_WQP_SO4_NY$daily_mean, probs = c(0.75)) # that would be 32,000
data_perc_75 = filter(SW_WQP_SO4_NY, daily_mean > Q_75)
...
ggsave(paste("./output/AGU/",state,"/figures/",type_w,"_dist_nearest_",well_type,"_",anal,".png", sep=""),
...
GW_WQP_SO4_NY = filter(GW_WQP_anal_daily, daily_mean < 3000000)

#Find 75th percentile? Then plot values above that value?
Q_75 = quantile(GW_WQP_SO4_NY$daily_mean, probs = c(0.75)) # that would be 32,000
data_perc_75 = filter(GW_WQP_SO4_NY, daily_mean > Q_75)
pal <- colorNumeric(palette = "viridis", domain = data_perc_75$daily_mean)
#map_75 = 
leaflet(data_perc_75) %>% addTiles() %>%
  addCircles(lng = data_perc_75$Longitude,
...
surface_water_data = SW_WQP_SO4_NY
groundwater_data = GW_WQP_SO4_NY
...
Geospatial_result_NY_SO4 = rbind(SW_marg,SW_orph,GW_marg,GW_orph)

saveRDS(Geospatial_result_NY_SO4,file = "Geospatial_results_NY_SO4.rds", compress = FALSE)
