...
SW_WQP_Na = readRDS("sw_wqp_wq_all_curated_Na.rds")

#Separate Texas samples
SW_WQP_Na_NY = filter(SW_WQP_Na, State=="New York")

#Calculate daily values
SW_WQP_Na_NY_daily = SW_WQP_Na_NY %>%
...
View(SW_WQP_Na_NY_daily)

SW_WQP_Na_NY = filter(SW_WQP_Na_NY_daily, daily_mean > 1 & daily_mean < 10000000)


#Mapping 
#Find 75th percentile? Then plot values above that value?
Q_75 = quantile(SW_WQP_Na_NY$daily_mean, probs = c(0.75)) # that would be 32,000
data_perc_75 = filter(SW_WQP_Na_NY, daily_mean > Q_75)
...
GW_WQP_Na = readRDS(file="gw_wqp_wq_all_curated_Na.rds")

#Separate Texas samples
GW_WQP_Na_NY = filter(GW_WQP_Na, State=="New York")

#Calculate daily values
GW_WQP_Na_NY_daily = GW_WQP_Na_NY %>%
  group_by(County, SiteCode, Latitude, Longitude, Samplingdate) %>%
  summarise(n = n(), daily_mean = mean(DataValue), daily_median = median(DataValue), 
            min_v = min(DataValue),max_v = max(DataValue)) %>%
  arrange(daily_mean)
...
#Check if there are any outliers
View(GW_WQP_Na_NY_daily)

GW_WQP_Na_NY = filter(GW_WQP_Na_NY_daily, daily_mean > 1 & daily_mean < 10000000)


#Mapping 
#Find 75th percentile? Then plot values above that value?
Q_75 = quantile(GW_WQP_Na_NY$daily_mean, probs = c(0.75)) # that would be 32,000
data_perc_75 = filter(GW_WQP_Na_NY, daily_mean > Q_75)
pal <- colorNumeric(palette = "viridis", domain = data_perc_75$daily_mean)
#map_75 = 
leaflet(data_perc_75) %>% addTiles() %>%
  addCircles(lng = data_perc_75$Longitude,
...
surface_water_data = SW_WQP_Na_NY
groundwater_data = GW_WQP_Na_NY
...
SW_marg = SW_marg_filt2 #replacing with filtered values
...
SW_orph_filt2 = SW_orph[-which(log(SW_orph$daily_mean) < 5 & log(SW_orph$nearest_distance_m) < 11),]
plot(log(SW_orph_filt2$nearest_distance_m), log(SW_orph_filt2$daily_mean))
cor(log(SW_orph_filt2$nearest_distance_m), log(SW_orph_filt2$daily_mean))

SW_orph = SW_orph_filt2
...
Geospatial_result_NY_Na = rbind(SW_marg,SW_orph,GW_marg,GW_orph)

saveRDS(Geospatial_result_NY_Na,file = "Geospatial_results_NY_Na.rds", compress = FALSE)
