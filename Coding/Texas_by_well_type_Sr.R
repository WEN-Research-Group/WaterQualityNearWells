...
#State level samples
TX_SW_Sr = read.csv("./input/Texas_surface_WQ/Strontium/TX_SW_Str.csv", header = TRUE)

#Analyze units
unique(TX_SW_Sr$Parameter.Description)
TX_SW_Sr = filter(TX_SW_Sr, Parameter.Description == "STRONTIUM, DISSOLVED, UG/L AS SR") 
...
Texas_state_Sr = Cleaning_TX_SW_fun_daily(TX_SW_Sr,TX_SW_sites)
Texas_state_Sr_non_USGS = filter(Texas_state_Sr,USGS_GAUGE == "")

#Check for outliers
Texas_state_Sr_non_USGS_org = Texas_state_Sr_non_USGS %>%
  arrange(daily_mean)
View(Texas_state_Sr_non_USGS_org)

#Remove values the largest value - clearly an outlier
Texas_state_Sr_non_USGS = Texas_state_Sr_non_USGS_org[-dim(Texas_state_Sr_non_USGS)[1],]
View(Texas_state_Sr_non_USGS)


#Water quality portal
#All data - need to clean it still
SW_WQP_Sr = readRDS("./input/sw_results_all_info_strontium.rds")

#Look up these to include in the filter
unique(SW_WQP_Sr$ResultDetectionConditionText)
unique(SW_WQP_Sr$MonitoringLocationTypeName)
#unique(SW_WQP_SO4$ResultStatusIdentifier)
unique(SW_WQP_Sr$ResultMeasure.MeasureUnitCode)

#Cleaning Na data for all the states
SW_WQP_Sr_clean = SW_WQP_Sr %>%
...
unique(SW_WQP_Sr_clean$ResultDetectionConditionText)
unique(SW_WQP_Sr_clean$MonitoringLocationTypeName)
unique(SW_WQP_Sr_clean$ResultStatusIdentifier)
unique(SW_WQP_Sr_clean$ResultMeasure.MeasureUnitCode)

#Do unit conversions 
ind = which(SW_WQP_Sr_clean$ResultMeasure.MeasureUnitCode == "mg/L"|
              SW_WQP_Sr_clean$ResultMeasure.MeasureUnitCode == "mg/l"|
              SW_WQP_Sr_clean$ResultMeasure.MeasureUnitCode == "ppm")

SW_WQP_Sr_clean[ind,"DataValue"] = SW_WQP_Sr_clean[ind,"DataValue"]*1000
#All values are converted and now ug/L
SW_WQP_Sr_clean$ResultMeasure.MeasureUnitCode = "ug/L"

#Save file to use with other states
saveRDS(SW_WQP_Sr_clean, file="sw_wqp_wq_all_curated_Sr.rds", compress = FALSE)

#Now analyze for a state
#Separate Texas samples
SW_WQP_Sr_Texas = filter(SW_WQP_Sr_clean, State=="Texas")

#Calculate daily values
SW_WQP_Sr_Texas_daily = SW_WQP_Sr_Texas %>%
...
View(SW_WQP_Sr_Texas_daily)
#Remove values above 10000000
SW_WQP_Sr_Texas_daily = filter(SW_WQP_Sr_Texas_daily, daily_mean < 100000 & daily_mean > 1)

#Merge data from WQP and Texas state database

#Change data types as needed
Texas_state_Sr_non_USGS$SiteCode = as.character(Texas_state_Sr_non_USGS$SiteCode)
#Remove some columns 
Texas_state_Sr_non_USGS = Texas_state_Sr_non_USGS[,-c(3,6)]

#Now merge
SW_Sr_Texas_merged = rbind(Texas_state_Sr_non_USGS,SW_WQP_Sr_Texas_daily)

#Save this for later
saveRDS(SW_Sr_Texas_merged,file = "SW_daily_Sr_Texas_merged.rds", compress = FALSE)


# GROUDWATER

#WQP
GW_WQP_Sr_clean = readRDS("gw_wqp_wq_all_curated_Sr.rds")

GW_WQP_Sr_clean_TX = filter(GW_WQP_Sr_clean, State == "Texas")

GW_WQP_Sr_clean_TX_daily = GW_WQP_Sr_clean_TX %>%
  group_by(County, SiteCode, Latitude, Longitude, Samplingdate) %>%
  summarise(n = n(), daily_mean = mean(DataValue), daily_median = median(DataValue), 
            min_v = min(DataValue),max_v = max(DataValue)) %>%
  arrange(daily_mean)
View(GW_WQP_Sr_clean_TX_daily)
#Remove values below 1 and above 2820
GW_WQP_Sr_clean_TX_daily = filter(GW_WQP_Sr_clean_TX_daily, daily_mean < 60000)
...
gw.twdb.sr_long_clean <- wqdetail %>%
  filter(WaterQualityParameterId %in% c(122, 123),#look up these values for each analyte
         is.na(Description),
         ParameterValue> 1) %>%
  mutate(SampleDate = 
  ...
  arrange(DataValue)
View(gw.twdb.sr_long_clean)

#All values gook fine

#Check units
unique(gw.twdb.sr_long_clean$ParameterLongDescription)

#No need to change units

# Reorder columns and drop some
gw_twdb_sr_clean = gw.twdb.sr_long_clean %>%
  select(c(County, SiteCode, Latitude, Longitude, Samplingdate, DataValue))

#Calculate daily averages 
TX_GW_TWDB_Sr_part1 = gw_twdb_sr_clean %>%
  group_by(County, SiteCode, Latitude, Longitude, Samplingdate) %>%
  summarise(n = n(), daily_mean = mean(DataValue), daily_median = median(DataValue), 
            min_v = min(DataValue),max_v = max(DataValue)) %>%
  ...
  arrange(daily_mean)
#Check for outliers
View(TX_GW_TWDB_Sr_part1)

#No need to remove anything
TX_GW_TWDB_Sr = TX_GW_TWDB_Sr_part1



#Now merge two datasets
GW_Sr_Texas_merged = rbind(TX_GW_TWDB_Sr,GW_WQP_Sr_clean_TX_daily)

#Save this for later
saveRDS(GW_Sr_Texas_merged,file = "GW_all_Sr_Texas_merged.rds", compress = FALSE)
...
surface_water_data = SW_Sr_Texas_merged
groundwater_data = GW_Sr_Texas_merged
...
#Test if function determining number of sampling sites near wells is working
source("Sampling_sites_near_wells.R")
nr_samp_nr_marg = Sampling_sites_near_wells(marginal_well_data,groundwater_data)

#Texas_marginal_short = marginal_well_data
#SW_Na_Texas_merged = marginal_well_data
...
plot(log(SW_unconv$num_well_1km[which(SW_unconv$num_well_1km>0)]), log(SW_unconv$daily_mean[which(SW_unconv$num_well_1km>0)]))
cor(log(SW_unconv$num_well_1km[which(SW_unconv$num_well_1km>0)]), log(SW_unconv$daily_mean[which(SW_unconv$num_well_1km>0)]))
...
hist(GW_marg$daily_mean)
#Pull out values between 1000 and 1200 - how many?
GW_marg_h_oc = filter(GW_marg, daily_mean > 999 & daily_mean < 1201) #7% of values are within this range
hist(GW_marg_h_oc$daily_mean)

GW_marg_nrs = merge(GW_marg, GW_marg_val_sum, by = "daily_mean")
GW_marg_nrs_f = filter(GW_marg_nrs, n_val < 49) 
plot(log(GW_marg_nrs_f$nearest_distance_m), log(GW_marg_nrs_f$daily_mean))
cor(log(GW_marg_nrs_f$nearest_distance_m), log(GW_marg_nrs_f$daily_mean)) #after removing exact values occuring over 50 times, corr coef didn't change much
...
hist((GW_orph$nearest_distance_m))
hist(log(GW_orph$nearest_distance_m))

########### SAVE ALL RESULTS
#Combine all 6 into 1 and save
Geospatial_result_TX_Sr = rbind(SW_marg,SW_unconv,SW_orph,GW_marg,GW_unconv,GW_orph)

saveRDS(Geospatial_result_TX_Sr,file = "Geospatial_results_TX_Sr.rds", compress = FALSE)
