...
SW_WQP_Na_PA = filter(SW_WQP_anal_daily, daily_mean < 2000000)


#SHALE NETWORK

#### Shale Network SC Data ####
Na_Shale_raw = read.csv(file="./input/NewShaleDownload/Na_0115.csv", na.strings = "", stringsAsFactors = FALSE)

unique(Na_Shale_raw$State)
unique(Na_Shale_raw$UnitsAbbreviation)
unique(Na_Shale_raw$VariableName) 
unique(Na_Shale_raw$SampleMedium)

Na_Shale_curated <- Na_Shale_raw %>%
...
View(Na_Shale_curated)

#Remove some of the categories
unique(Na_Shale_curated$SiteType)
unique(Na_Shale_curated$SampleMedium)


# extract groundwater and surface water samples

sw.sn_curated_all <- Na_Shale_curated %>%
...
SW_Shale_Na_PA = filter(SW_Shale_anal_daily, daily_mean < 100000)


#Merge two datasets together and save
SW_Na_PA_merged = rbind(SW_Shale_Na_PA, SW_WQP_Na_PA)

SW_Na_PA_merged = SW_Na_PA_merged %>%
  arrange(daily_mean)
View(SW_Na_PA_merged)

#Remove values below 1700000
SW_Na_PA_merged = filter(SW_Na_PA_merged, daily_mean < 1700000)

#Save this for later
saveRDS(SW_Na_PA_merged,file = "SW_all_Na_Penn_merged_0115.rds", compress = FALSE)


# Now it's clean
SW_Na_PA_merged_org = SW_Na_PA_merged %>%
  arrange(daily_mean)
View(SW_Na_PA_merged_org)
...
Q_75 = quantile(SW_Na_PA_merged_org$daily_mean, probs = c(0.75)) # that would be 32,000
data_perc_75 = filter(SW_Na_PA_merged_org, daily_mean > Q_75)
...
# SHALE NETWORK
gw.sn_curated_all <- Na_Shale_curated %>%
...
GW_Shale_Na_PA = filter(GW_Shale_anal_daily, daily_mean < 1000000)
...
#Remove a few largest values
GW_WQP_Na_PA = filter(GW_WQP_anal_daily, daily_mean < 6000000)

#Merge two datasets together and save
GW_Na_PA_merged = rbind(GW_Shale_Na_PA, GW_WQP_Na_PA)

GW_Na_PA_merged = GW_Na_PA_merged %>%
  arrange(daily_mean)
View(GW_Na_PA_merged)

#Remove values above 4000000
GW_Na_PA_merged = filter(GW_Na_PA_merged, daily_mean < 4000000)

#Save this for later
saveRDS(GW_Na_PA_merged,file = "GW_all_Na_Penn_merged_0115.rds", compress = FALSE)


# Now it's clean
GW_Na_PA_merged_org = GW_Na_PA_merged %>%
  arrange(daily_mean)
View(GW_Na_PA_merged_org)
#ind = which(SW_SC_PA_merged_org$Longitude > 0)
#SW_SC_PA_merged_org$Longitude[ind] = SW_SC_PA_merged_org$Longitude[ind] *(-1)
#Find 75th percentile? Then plot values above that value?
Q_75 = quantile(GW_Na_PA_merged_org$daily_mean, probs = c(0.75)) # that would be 32,000
data_perc_75 = filter(GW_Na_PA_merged_org, daily_mean > Q_75)
pal <- colorNumeric(palette = "viridis", domain = data_perc_75$daily_mean)
#map_75 = 
leaflet(data_perc_75) %>% addTiles() %>%
  addCircles(lng = data_perc_75$Longitude,
...
dim(GW_Na_PA_merged_org)
Q_75



#Upload SW and GW data
SW_Na_PA_merged = readRDS("SW_all_Na_Penn_merged_0115.rds")
GW_Na_PA_merged = readRDS("GW_all_Na_Penn_merged_0115.rds")
...
surface_water_data = SW_Na_PA_merged
groundwater_data = GW_Na_PA_merged
